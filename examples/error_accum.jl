...
opts = (FullQR = Dict(:kernel => :FullQR),
        ...
        CholeskyUpDown_20pctQR = Dict(:kernel => :CholeskyUpDown, :pct_full_qr => 20.0),
        CholeskyUpDown_50pctQR = Dict(:kernel => :CholeskyUpDown, :pct_full_qr => 50.0),
)
...
for (N,size) in zip(Ns, sizes)
    V =  rand(size...)
    w_in = rand(size[1])
    plt = plot(ylims=(1e-16, 1e2), yticks=10.0 .^ (-16:2:2), yaxis=:log, leg=:outerright, title="Accumulated errors N=$N", size=(800,400))
