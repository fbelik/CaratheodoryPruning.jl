...
function caratheodory_pruning(V, w_in, caratheodory_correction=false; kernel=:CholeskyDowndater, pruning=:first, return_errors=false, errnorm=norm, zero_tol=1e-16, progress=false, kernel_kwargs...)
    ...
    return caratheodory_pruning(V, w_in, kernel_downdater, prune_weights!, caratheodory_correction, return_errors=return_errors, errnorm=errnorm, zero_tol=zero_tol, progress=progress)
end